        ...
        if (type instanceof ParameterizedType) {//自定义ApiResult
            ...
            if (ApiResult.class.isAssignableFrom(cls)) {
                ...
                try {
                    ...
                    if (!List.class.isAssignableFrom(rawType) && clazz.equals(String.class)) {
                        apiResult.setData((T) responseBodyStr);
                        apiResult.setCode(0);
                       /* final Type type = Utils.getType(cls, 0);
                        ...
                        ApiResult result = gson.fromJson(responseBodyStr, type);
                        if (result != null) {
                            apiResult = result;
                        } else {
                            apiResult.setMsg("json is null");
            ...
            try {
                ...
                if (clazz.equals(String.class)) {
                    //apiResult.setData((T) json);
                    //apiResult.setCode(0);
                    final ApiResult result = parseApiResult(responseBodyStr, apiResult);
                    if (result != null) {
                        apiResult = result;
                        apiResult.setData((T) responseBodyStr);
                    } else {
